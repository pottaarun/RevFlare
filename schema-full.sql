...
CREATE TABLE IF NOT EXISTS persona_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, account_id INTEGER NOT NULL, persona TEXT NOT NULL, message_type TEXT NOT NULL, subject TEXT, content TEXT NOT NULL, approval_status TEXT DEFAULT 'pending_approval', variation_group TEXT, variation_index INTEGER DEFAULT 0, variation_label TEXT, user_email TEXT DEFAULT '', created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (account_id) REFERENCES accounts(id) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS campaign_emails (id INTEGER PRIMARY KEY AUTOINCREMENT, campaign_id INTEGER NOT NULL, account_id INTEGER NOT NULL, account_name TEXT, subject TEXT, content TEXT, status TEXT DEFAULT 'pending', approval_status TEXT DEFAULT 'pending_approval', variation_group TEXT, variation_index INTEGER DEFAULT 0, variation_label TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE);
